...
int main(int ac, char **av, char **env)
{
    ...
    {   
        ...
        line[_strlen(line) - 1] = '\0'; args = strtow(line);
        if (get_op_func(args[0]) != NULL)
        {
            ...
            free_args(args); free(path); continue;
        }
        ...
        free(args[0]); args[0] = fullPath; frk = fork();
        if (frk == 0)
        {
            execve(args[0], args, env); perror("./shell");
            free_args(args); free(fullPath); free(path); _exit(EXIT_FAILURE);
        }    
        else if (frk == -1)
            perror("./shell");
        wait(&status); free_args(args); free(path);
    } 
    return (0);
}
